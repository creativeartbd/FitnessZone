 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('fitnesszone_features')) {

	function fitnesszone_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'fitnesszone-gallery-footer-widget', 85, 64, true  ); //gallery widget thumb
			add_image_size( 'fitnesszone-gallery-ii-column', 585, 392, true  ); 	// gallery - ii column
			add_image_size( 'fitnesszone-gallery-iii-column', 420, 287, true  ); 	// gallery - iii column
			add_image_size( 'fitnesszone-gallery-iv-column', 420, 287, true  ); 	// gallery - iv column
			add_image_size( 'fitnesszone-gallery-iii&iv-fullwidth', 780, 780, true  ); 	// gallery - iii&iv column
			
			add_image_size( 'fitnesszone-blog-i-column', 1168, 600, true  ); 	// blog - i column
			add_image_size( 'fitnesszone-blog-ii-column', 568, 388, true  ); 	// blog - ii column
			add_image_size( 'fitnesszone-blog-iii-column', 439, 300, true  ); 	// blog - iii column
			add_image_size( 'fitnesszone-blog-thumb', 439, 300, true  ); 	// blog - thumb
			add_image_size( 'fitnesszone-blog-list', 600, 400, true  ); 	// blog - list
			add_image_size( 'fitnesszone-blog-ii-column-masonry', 750 ); 	// blog - ii column masonry
			add_image_size( 'fitnesszone-blog-iii-column-masonry', 540 ); 	// blog - iii column masonry
			
			add_image_size( 'fitnesszone-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'fitnesszone-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'fitnesszone-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'fitnesszone-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'fitnesszone-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list
		...
		register_nav_menus( array(
			'main-menu' => esc_html__('Main Menu', 'fitnesszone'),
		) );
		...
		$skin = get_theme_mod('use-predefined-skin',fitnesszone_defaults( 'use-predefined-skin' ) );

		if( $skin ) {
			$skin            = get_theme_mod('predefined-skin',fitnesszone_defaults( 'predefined-skin' ) );
			$colors          = fitnesszone_skins( $skin );
		...
		add_theme_support( 'editor-color-palette', array(
			array(
				'name'  => esc_html__( 'Primary Color', 'fitnesszone' ),
				'slug'  => 'primary',
				'color' => $primary_color,
			),
			array(
				'name'  => esc_html__( 'Secondary Color', 'fitnesszone' ),
				'slug'  => 'secondary',
				'color' => $secondary_color,
			),
			array(
				'name'  => esc_html__( 'Tertiary Color', 'fitnesszone' ),
				'slug'  => 'tertiary',
				'color' => $tertiary_color,
			)
		));
	}
	add_action('after_setup_theme', 'fitnesszone_features');
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('fitnesszone_under_construction') ){
	function fitnesszone_under_construction(){
...
if( cs_get_option( 'enable-comingsoon' ) ):
	add_action('template_redirect', 'fitnesszone_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'fitnesszone_rand_css', 101);
	function fitnesszone_rand_css() {
		$id = cs_get_option( 'comingsoon-pageid' );
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, FITNESSZONE_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('fitnesszone_wp_list_categories') ){
	function fitnesszone_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'fitnesszone_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('fitnesszone_wp_list_archive') ){
	function fitnesszone_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'fitnesszone_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('fitnesszone_wpcf7_form_elements') ){
	function fitnesszone_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'fitnesszone_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Pagination for Blog and Gallery
 * --------------------------------------------------------------------------- */
function fitnesszone_pagination( $query = false, $load_more = false ){
	global $wp_query;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'fitnesszone') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'fitnesszone'),
					'next_text'          => esc_html__('Next', 'fitnesszone').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
...
function fitnesszone_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'fitnesszone' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'fitnesszone'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'fitnesszone' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'fitnesszone' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'fitnesszone' );
		}
		...
		$title = esc_html__('Tag Archives','fitnesszone');
	}
	return $title;
}

#Get Event's Venue Address...
	function fitnesszone_event_venue_details($event_ID) {
		...
		$out = '<p>'.$venue_all.'&nbsp; <a href="https://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q='.urlencode($venue_all).'"><span class="fa fa-map-marker"></span> '. esc_html__('Google Map','fitnesszone').'</a></p>';
		return $out;
	}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function fitnesszone_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function fitnesszone_wp_kses($content) {
 ...
 */
function fitnesszone_before_after_widget ( $content ) {
 ...
 */
function fitnesszone_widget_title( $content ) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('fitnesszone_hex2rgb')) {

	function fitnesszone_hex2rgb($hex) {
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function fitnesszone_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'fitnesszone');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'fitnesszone'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo '	</div>';
		echo '		<div class="commentmetadata"> '.get_the_date ( get_option('date_format') ).' at '.get_the_time ( get_option('time_format') ).'</div>';
		echo '  <div class="comment-body">';
					comment_text();
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'fitnesszone');
					endif;
					edit_comment_link(esc_html__('Edit', 'fitnesszone'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'fitnesszone'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * --------------------------------------------------------------------------- */
function fitnesszone_get_page_permalink_by_its_template( $template ) {
	$permalink = '#';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function fitnesszone_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function fitnesszone_global_variables($variable = '') {
...
/* ---------------------------------------------------------------------------
 * Walker Page for fitnesszone_new_wp_page_menu
 * --------------------------------------------------------------------------- */
class FITNESSZONE_Walker_Page extends Walker_Page {
 ...
 * --------------------------------------------------------------------------- */
add_filter('upload_mimes', 'fitnesszone_upload_mimes');
function fitnesszone_upload_mimes( $existing_mimes = array() ){
