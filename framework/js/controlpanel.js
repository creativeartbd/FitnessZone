...
jQuery(document).ready(function($){
	...
	if($.cookie("fitnesszone_skin")!= null){
		var $href = dttheme_urls.theme_base_url + '/css/skins/' + $.cookie("fitnesszone_skin")+"/style.css";
		$("link[id='fitnesszone-skin-css']").attr("href",$href);
		$("ul.color-picker a[id='"+$.cookie("fitnesszone_skin")+"']").addClass("selected");
	}else{
		$("ul.color-picker a:first").addClass("selected");
	}

	//Apply Layout
	if($.cookie("fitnesszone_layout") == "boxed"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("fitnesszone_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");

		$i = ($.cookie("fitnesszone_pattern")) ? $.cookie("fitnesszone_pattern")  : 'pattern1';
		...
		$("ul.pattern-picker a[id="+$.cookie("fitnesszone_pattern")+"]").addClass('selected');
	} else if($.cookie("fitnesszone_layout") == "fullwidth"){
		$("ul.layout-picker li a").removeAttr("class");
		$("ul.layout-picker li a[id='"+$.cookie("fitnesszone_layout")+"']").addClass("selected");
		$("div#pattern-holder").removeAttr("style");
	...
	if($.cookie("fitnesszone_scheme") != null){
		$("ul.scheme-picker li a").removeAttr("class");
		if( $.cookie("fitnesszone_scheme") === "dark" ) {
			$("<link id='light-dark-css' href='"+ dttheme_urls.theme_base_url +"/css/dark-skin.css' rel='stylesheet' media='all' />").insertBefore($('#fitnesszone-gutenberg-css'));
			$("ul.scheme-picker a:last").addClass('selected');
		}
		else if( $.cookie("fitnesszone_scheme") === "light" ) {
	...
	$("ul.layout-picker a").on('click', function(e){
		...
		$.cookie("fitnesszone_layout", $this.attr("id"), { path: '/' });

		if( $.cookie("fitnesszone_layout") === "boxed") {
			$("body").removeClass("layout-wide").addClass("layout-boxed");
			$("div#pattern-holder").slideDown();
			
			if( $.cookie("fitnesszone_pattern") == null ){
				$("ul.pattern-picker a:first").addClass('selected');
				$.cookie("fitnesszone_pattern","pattern1",{ path: '/' });
			}else{
				$("ul.pattern-picker a[id="+$.cookie("fitnesszone_pattern")+"]").addClass('selected');
				$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("fitnesszone_pattern")+".jpg";
	...
	$("ul.scheme-picker a").on('click', function(e){
		...
		$this.addClass("selected");
		$.cookie("fitnesszone_scheme", $this.attr("id"), { path: '/' });
		if( $.cookie("fitnesszone_scheme") === "dark" ) {
			$("<link id='light-dark-css' href='"+ dttheme_urls.theme_base_url +"/css/dark-skin.css' rel='stylesheet' media='all' />").insertBefore($('#fitnesszone-gutenberg-css'));
		}
		else if( $.cookie("fitnesszone_scheme") === "light" ) {
	...
	$("ul.pattern-picker a").on('click', function(e){
		if($.cookie("fitnesszone_layout") == "boxed"){
			...
			$("ul.pattern-picker a").removeAttr("class");
			$this.addClass("selected");
			$.cookie("fitnesszone_pattern", $this.attr("id"), { path: '/' });
			$img = dttheme_urls.theme_base_url + "/images/patterns/"+$.cookie("fitnesszone_pattern")+".jpg";
			$('body').css('background-image', 'url('+$img+')');
		}
		e.preventDefault();
	...
	$("ul.color-picker a").on('click', function(e){
		...
		$this.addClass("selected");
		$.cookie("fitnesszone_skin", $this.attr("id"), { path: '/' });
